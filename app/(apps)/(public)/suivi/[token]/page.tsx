      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        <div className={"rounded-2xl p-5 text-center border " + (step === 2 ? "bg-emerald-50 border-emerald-200" : step === 1 ? "bg-blue-50 border-blue-200" : "bg-amber-50 border-amber-200")}>
          <div className={"w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 " + (step === 2 ? "bg-emerald-100" : step === 1 ? "bg-blue-100" : "bg-amber-100")}>
            {step === 2 ? <TbTruck size={28} className="text-emerald-600" /> : step === 1 ? <TbCheck size={28} className="text-blue-600" /> : <TbClock size={28} className="text-amber-600" />}
          </div>
            ...
            return (
              <div key={s.key} className="flex gap-4">
                <div className="flex flex-col items-center">
                  <div className={"w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 border-2 " + (done ? "bg-[#2B3090] border-[#2B3090]" : "bg-white border-gray-200")}>
                    {done ? <TbCheck size={16} className="text-white" /> : <s.icon size={16} className="text-gray-300" />}
                  </div>
                  {i < STEPS.length - 1 && <div className={"w-0.5 h-10 " + (i < step ? "bg-[#2B3090]" : "bg-gray-100")} />}
                </div>
                <div className="pb-8">
                  <p className={"font-bold text-sm " + (done ? "text-gray-900" : "text-gray-300")}>{s.label}</p>
                  <p className={"text-xs mt-0.5 " + (i === step ? "text-[#2B3090] font-medium" : done ? "text-gray-400" : "text-gray-200")}>{s.desc}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm space-y-4">
          ...
          {product && (
            <div className="flex items-center gap-3">
              {product.images?.[0] ? <img src={product.images[0]} alt="" className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-gray-400">Client</span><span className="font-semibold text-gray-800">{order.buyer_name}</span></div>
            <div className="flex justify-between"><span className="text-gray-400">Livraison</span><span className="font-semibold text-gray-800">{order.delivery_address}</span></div>
        ...
        {vendor && (
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-[#2B3090]/10 flex items-center justify-center font-bold text-[#2B3090] overflow-hidden flex-shrink-0">
