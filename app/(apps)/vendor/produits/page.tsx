...
import { TbPlus, TbPackage, TbPencil, TbTrash, TbLoader2, TbEye, TbSearch, TbPhoto, TbLock, TbCreditCard } from "react-icons/tb";
import { useCartStore } from "@/lib/zustand/cart-store";
import ConfirmModal from "@/components/ui/ConfirmModal";

const fmt = (p: number) => new Intl.NumberFormat("fr-FR").format(p) + " FCFA";

type Product = { id: string; name: string; price: number; category: string; stock: number; status: string; images: string[] };
...
export default function MesProduits() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setSuspended(!isOk);
      const { data } = await supabase.from("products").select("id, name, price, category, stock, status, images").eq("vendor_id", vid).order("created_at", { ascending: false });
      setProducts(data || []);
      setLoading(false);
    })();
  }, []);
    ...
    <div className="max-w-5xl mx-auto space-y-6">
                  ...
                  <tr key={p.id} className={"transition-colors " + (suspended ? "bg-gray-50/50" : "hover:bg-gray-50")}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 justify-end">
                        <Link href={"/produits/"+p.id} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-blue-50 hover:text-blue-500 transition-colors"><TbEye size={15} /></Link>
                        {suspended ? <div title="Abonnement requis" className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-300 cursor-not-allowed"><TbLock size={15} /></div> : <Link href={"/vendor/produits/"+p.id+"/modifier"} className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-orange-50 hover:text-orange-500 transition-colors"><TbPencil size={15} /></Link>}
                        <button onClick={() => setConfirmDelete(p.id)} disabled={suspended} title={suspended ? "Abonnement requis" : "Supprimer"} className={"w-8 h-8 rounded-lg flex items-center justify-center transition-colors " + (suspended ? "bg-gray-50 text-gray-300 cursor-not-allowed" : "bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-500")}>
                          {deleting === p.id ? <TbLoader2 size={15} className="animate-spin" /> : suspended ? <TbLock size={15} /> : <TbTrash size={15} />}
                        </button>
                      </div>
                    </td>
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          subMessage="Il sera retire du panier. Votre boutique reste intacte."
