...
const features = [
  ...
  { icon: TbHeadset, title: "Support rapide", description: "Reponse en moins d'1h" },
];

const stats = [
  { value: "500+", label: "Produits" },
  { value: "50+", label: "Vendeurs" },
  { value: "1 000+", label: "Etudiants" },
      ...
      <div className="relative bg-gradient-to-br from-[#1A1F6B] via-[#2B3090] to-[#3D44B5] overflow-hidden">
        ...
        <MainLayout className="relative z-10 py-12 md:py-16">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            {/* Colonne droite — sidebar categories */}
                    ...
                    <li key={index}>
                      <Link
                        href={index < categories.length - 1
                          ? `/produits?categorie=${encodeURIComponent(item.name)}`
                          : "/produits"}
                        className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors hover:bg-white/10 ${
                          index === categories.length - 1
                            ? "text-[#F5A623] font-bold border-t border-white/10 mt-1"
                            : "text-white/80"
                        }`}
                      >
