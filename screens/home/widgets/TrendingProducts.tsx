  ...
  return (
    <Link href={`/produits/${p.id}`}
      ...
      <div className="h-40 bg-gray-100 flex items-center justify-center overflow-hidden relative">
        ...
        {p.category && (
          <span className="absolute top-2 left-2 bg-white/90 text-gray-600 text-[10px] font-semibold px-2 py-0.5 rounded-full z-10">
            {p.category}
          </span>
        )}
      </div>
      <div className="p-3">
        ...
        {p.promo_price && p.promo_ends_at && new Date(p.promo_ends_at) > new Date() ? (
          <div className="flex items-center gap-1.5 mt-1">
            ...
            <span className="bg-red-100 text-red-500 text-[9px] font-black px-1.5 py-0.5 rounded-full">
    ...
    <section className="px-4 py-10 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
          <button onClick={() => setTab("recent")}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === "recent" ? "bg-white text-[#2B3090] shadow-sm" : "text-gray-500"}`}>
            <TbClock size={15} /> Nouveaux
          </button>
          <button onClick={() => setTab("popular")}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === "popular" ? "bg-white text-[#2B3090] shadow-sm" : "text-gray-500"}`}>
            <TbTrendingUp size={15} /> Populaires
          </button>
        </div>
        <Link href="/produits" className="text-xs text-[#2B3090] font-semibold hover:underline flex items-center gap-1">
      ...
      {loading ? <SkeletonGrid /> : displayed.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link href="/devenir-vendeur" className="text-[#2B3090] text-sm font-semibold hover:underline mt-2 block">
