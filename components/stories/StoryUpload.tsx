...
export default function StoryUpload({ vendorId, vendorName, onUploaded }: StoryUploadProps) {
  ...
  const upload = async () => {
    ...
    const { data: urlData } = supabase.storage.from("vendor-stories").getPublicUrl(path);
  ...
  return (
    <>
      ...
      <button onClick={() => setOpen(true)}
        ...
        <div className="w-16 h-16 rounded-full border-2 border-dashed border-[#2B3090]/40 flex items-center justify-center text-[#2B3090] group-hover:border-[#2B3090] group-hover:bg-[#2B3090]/5 transition-all">
          <TbPlus size={24} />
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-[#1A1F6B] to-[#2B3090] p-5">
            ...
            <div className="p-5 space-y-4">
              {done ? (
                ...
                <>
                  ...
                  {!preview ? (
                    <div
                      className="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center cursor-pointer hover:border-[#2B3090]/40 transition-colors"
                  ...
                  <div>
                    ...
                    <input
                      className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#2B3090]/20"
                  ...
                  <button onClick={upload} disabled={!file || uploading}
                    className="w-full bg-gradient-to-r from-[#F5A623] to-[#e05c1a] text-white py-3.5 rounded-xl font-bold text-sm disabled:opacity-50 flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
